...
<body>
	...
	<article class="panel">
		<div class="panel__copy_maxi">
			<h3 class="panel__copy__title">Per ulteriori informazioni contatta per email il responsabile</h3>
			...
			<table>
				<tr>
					<td>Nome</td>
					<td>Carattere</td>
					<td>Adozione ideale</td>
					<td>Email responsabile</td>
				</tr>
	...
	<?php
			...
			$query = "SELECT nomeC, carattere, adozione_ideale, volontarioEm
		  ...
		  // controllo sul numero dei record coinvolti
			...
			while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
			{
				...
				echo '<td>' . $row['carattere'] . '</td>';
				echo '<td>' . $row['adozione_ideale'] . '</td>';
				echo '<td>' . $row['volontarioEm'] . '</td>';
				echo '</tr>';
			}
			}
